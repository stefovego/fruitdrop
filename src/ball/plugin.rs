            ...
            CollisionEvent::Started(first_entity, second_entity, _) => {
                for entity in [first_entity, second_entity] {
                    if let Ok(entity) = ball_query.get(*entity) {
                        let other_entity = if entity == *first_entity {
                            second_entity
                        } else {
                            first_entity
                        };
                ...
                if let (Ok((f, ft, first_transform)), Ok((s, st, second_transform))) = (
                    ...
                    if *ft == *st {
                        ...
                        if *ft != KING_BALL {
                            ...
                            commands
                                .spawn(ball::bundles::new(mesh_material, og_ball_type.upgraded))
                                .insert(GrowTimer {
...
fn spawn_ball(
    ...
    if let Ok(transform) = dropper_query.get_single() {
        if keyboard_input.just_pressed(KeyCode::Space) {
            ...
            commands
                .spawn(ball::bundles::new(mesh_material, loadedball.balltype))
                .insert(DropTimer {
